...
class FlyTokenWebSource extends TokenWebSource
{
    ...
    public function getFilePathFromUrl($url)
    {
        ...
        if (count($parts) < 2) {
            throw new FileNotFoundException('`' . $subPath . '` - (wrong number of path parts)');
        }
        ...
        if (time() - $timestamp > $ttl) {
            throw new FileNotFoundException('`' . $subPath . $file . '` - (file link expired)');
        }
        if ($token != $correctToken) {
            throw new FileNotFoundException('`' . $subPath . $file . '` - (wrong token)');
        }
