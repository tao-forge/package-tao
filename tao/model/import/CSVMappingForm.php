 ...
 */
class CSVMappingForm extends tao_helpers_form_FormContainer
{
    // --- ASSOCIATIONS ---
    ...
    public function initElements()
    {
        ...
        if (
            ...
            foreach ($this->options['csv_column'] as $i => $column) {
                $columnsOptions[$i . CsvImporter::OPTION_POSTFIX] = __('Column') . ' ' . ($i + 1) . ' : ' . $column;
                $columnsOptionsLabels[$i . CsvImporter::OPTION_POSTFIX] = $this->prepareString($column);
            }
        } else {
            ...
            for ($i = 0; $i < count($this->options['csv_column']); $i++) {
                $columnsOptions[$i . CsvImporter::OPTION_POSTFIX] = __('Column') . ' ' . ($i + 1);
            }
        }
        ...
        if (isset($this->options[CsvUploadForm::IS_OPTION_FIRST_COLUMN_ENABLE])) {
            if ($this->options[CsvUploadForm::IS_OPTION_FIRST_COLUMN_ENABLE] === true) {
