...
class ServiceCallHelper
{
    ...
    public static function getInputValues(ServiceCall $serviceCall, $callParameters)
    {
        $returnValue = [];
        foreach ($serviceCall->getInParameters() as $param) {
            $paramKey = self::getParamName($param->getDefinition());
            switch (get_class($param)) {
                case 'oat\\tao\\model\\service\\ConstantParameter':
                    $returnValue[$paramKey] = $param->getValue();
                    break;
                case 'oat\\tao\\model\\service\\VariableParameter':
