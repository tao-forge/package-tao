 ...
 *
 * Service created from \oat\taoQtiTest\helpers\Utils static helper class
 *
...
class QtiTestUtils extends ConfigurableService
{
     ...
     * @param \oat\taoQtiTest\models\ManifestParser $manifestParser A Manifest Parser.
     ...
     */
    public function buildAssessmentItemRefsTestMap(XmlDocument $test, ManifestParser $manifestParser, $basePath)
    {
        $assessmentItemRefs = $test->getDocumentComponent()->getComponentsByClassName('assessmentItemRef');
        $map = ['items' => [], 'dependencies' => []];
        $itemResources = $manifestParser->getResources(Resource::getItemTypes(), ManifestParser::FILTER_RESOURCE_TYPE);
            ...
            foreach ($itemResources as $itemResource) {
                ...
                if ($itemResourceCanonicalHref == $itemRefCanonicalHref && is_file($itemResourceCanonicalHref)) {
                    ...
                    foreach ($itemResource->getDependencies() as $dependencyIdentifier) {
                        /** @var \oat\taoQtiTest\models\QtiResource $resource */
