 ...
 */
class QtiTestCompiler extends TestCompiler implements ContainerProvider
{
    ...
    protected function initCompilation()
    {
        ...
        // Extra path.
        $testService = QtiTestService::singleton();
        ...
        $renderingEngine->setStateName(QtiTestService::TEST_RENDERING_STATE_NAME);
        $renderingEngine->setRootBase(QtiTestService::TEST_PLACEHOLDER_BASE_URI . rtrim($this->getExtraPath(), $ds));
        $renderingEngine->setViewsName(QtiTestService::TEST_VIEWS_NAME);
     ...
     * @throws oat\taoQtiTest\models\QtiTestCompilationFailedException If an error occurs during the compilation.
     */
    public function compile()
    {
        ...
        try {
            ...
            if ($itemReport->getType() != common_report_Report::TYPE_SUCCESS) {
                ...
                $code = QtiTestCompilationFailedException::ITEM_COMPILATION;
                throw new QtiTestCompilationFailedException($msg, $this->getResource(), $code);
            }
    ...
    protected function compactTest()
    {
        ...
        $resolver = new ItemResolver(Service::singleton());
     ...
     * @throws oat\taoQtiTest\models\QtiTestCompilationFailedException If the test does not refer to at least one item.
     * @return common_report_Report
     */
    ...
    protected function buildServiceCall()
    {
        ...
        $param = new ConstantParameter(
            ...
            new core_kernel_classes_Resource(QtiTestService::INSTANCE_FORMAL_PARAM_TEST_DEFINITION),
            $this->getResource()
        );
        ...
        $param = new ConstantParameter(
            ...
            new core_kernel_classes_Resource(QtiTestService::INSTANCE_FORMAL_PARAM_TEST_COMPILATION),
    ...
    protected function compileRubricBlocks(AssessmentTest $assessmentTest)
    {
        ...
        foreach ($rubricBlockRefs as $rubricRef) {
            ...
            $mainStringRendering = str_replace(QtiTestService::TEST_PLACEHOLDER_BASE_URI, '<?php echo $' . QtiTestService::TEST_BASE_PATH_NAME . '; ?>', $mainStringRendering);
     ...
     * @throws oat\taoQtiTest\models\QtiTestCompilationFailedException If a remote resource cannot be retrieved.
     */
    protected function copyRemoteResources(RubricBlock $rubricBlock)
    {
        $ds = DIRECTORY_SEPARATOR;
        $tmpDir = tao_helpers_File::createTempDir();
        $destPath = trim($this->getExtraPath(), $ds) . $ds . QtiTestService::TEST_REMOTE_FOLDER . $ds;
        ...
        foreach ($search as $component) {
            ...
            if (isset($url) && !preg_match('@^' . ROOT_URL . '@', $url) && !Url::isRelative($url)) {
                ...
                if ($tmpFile !== false) {
                    ...
                    fclose($handle);
                    unlink($tmpFile);
                    $newUrl =  QtiTestService::TEST_REMOTE_FOLDER . '/' . $pathinfo['basename'];
                    ...
                    throw new QtiTestCompilationFailedException($msg, $this->getResource(), QtiTestCompilationFailedException::REMOTE_RESOURCE);
                }
            }
        }
    }
    ...
    protected function compileTest(AssessmentTest $test)
    {
        ...
        $this->getServiceLocator()->get(CompilationDataService::SERVICE_ID)->writeCompilationData(
            ...
            QtiTestService::TEST_COMPILED_FILENAME,
            $test
        );
    ...
    protected function compileTestMetadata(core_kernel_classes_Resource $resource)
    {
        ...
        $this->getPrivateDirectory()->write(QtiTestService::TEST_COMPILED_METADATA_FILENAME, json_encode($metadataJson));
    }
    ...
    protected function compileIndex()
    {
        ...
        if ($index) {
            $compiledDocDir->write(QtiTestService::TEST_COMPILED_INDEX, $index->serialize());
        }
    }
    ...
    {
        return QtiTestService::TEST_COMPILED_HREF_INDEX_FILE_PREFIX . md5($identifier) . QtiTestService::TEST_COMPILED_HREF_INDEX_FILE_EXTENSION;
    }
