 ...
 */
class QtiTestService extends TestService
{
    ...
    public function save(core_kernel_classes_Resource $test, array $items)
    {
        try {
            ...
            throw new QtiTestServiceException(
                "An error occured while dealing with the QTI-XML test: " . $e->getMessage(),
                QtiTestServiceException::TEST_WRITE_ERROR
            );
        }
     ...
     * @param oat\taoQtiTest\models\ManifestParser $manifestParser The parser used to retrieve the IMS Manifest.
     ...
     */
    protected function importTest(core_kernel_classes_Class $targetClass, Resource $qtiTestResource, ManifestParser $manifestParser, $folder, array $ignoreQtiResources = [])
    {
        ...
        if (is_readable($expectedTestFile) === false) {
            ...
            try {
                ...
                $dependencies = Utils::buildAssessmentItemRefsTestMap($testDefinition, $manifestParser, $folder);
     ...
     * @throws QtiTestServiceException If an unexpected runtime error occurs.
     */
    ...
    protected function importTestAuxiliaryFiles(Directory $testContent, Resource $qtiResource, $extractionFolder, common_report_Report $report)
    {
        ...
        foreach ($qtiResource->getAuxiliaryFiles() as $aux) {
            try {
                Utils::storeQtiResource($testContent, $aux, $extractionFolder);
    ...
    public function getTestFile(core_kernel_classes_Resource $test)
    {
        ...
        if (is_null($testModel) || $testModel->getUri() != self::INSTANCE_TEST_MODEL_QTI) {
            throw new QtiTestServiceException(
                ...
                QtiTestServiceException::TEST_READ_ERROR
            );
        }
     ...
     * @throws QtiTestServiceException If a runtime error occurs while creating the test content.
     */
    public function createContent(core_kernel_classes_Resource $test, $createTestFile = true, $preventOverride = true)
    {
        ...
        if ($createTestFile === true) {
            ...
            if (!$file->update($doc->saveXML())) {
                ...
                throw new QtiTestServiceException($msg, QtiTestServiceException::TEST_WRITE_ERROR);
            }
        }
